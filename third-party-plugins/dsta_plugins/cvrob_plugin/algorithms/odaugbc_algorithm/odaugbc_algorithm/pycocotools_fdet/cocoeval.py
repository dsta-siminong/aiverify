...
class COCOeval:
        ...
        for k, k0 in enumerate(k_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for imageDict in evalImgs:
                    ...
                    filteredArr = np.where(finalMask==True, imageDict['dtMatches'], -np.ones((T,1)))
    ...
    def generateReport(self, beta=1, iouThr=0.5, confThr=0):
        """
        Generate a sklearn-like classification report as a dictionary.
        ...
        tpCum, fpCum, fnCum, numGtCum = self._filterCum(
            iouThr,
            confThr=confThr
        )

        precisions, recalls = self._calculatePrecisionRecall(
            tpCum,
            fpCum,
            fnCum,
            numGtCum
        ...
        fscores = self._calculateFBetaScore(
            beta,
            precisions,
            recalls,
            tpCum,
        ...
        # Per-class metrics
        for cls_name, precision, recall, fscore, support in zip(
            catNms,
            precisions.flatten(),
            recalls.flatten(),
            fscores.flatten(),
        ...
        # # Micro average
        # micro_precision, micro_recall = self._calculatePrecisionRecall(
        #     tpCum,
        #     fpCum,
        #     fnCum,
        #     numGtCum,
        ...
        # micro_fscore = self._calculateFBetaScore(
        #     beta,
        #     micro_precision,
        #     micro_recall,
        #     sum(tpCum),
        ...
        #     f"f{beta}-score": float(micro_fscore),
        ...
        #     f"f{beta}-score": float(np.mean(fscores)),
        #     "support": total_support,
        # }

        # # Weighted average
        # weighted_precision = (
        #     sum(p * s for p, s in zip(precisions, numGtCum)).item()
        #     / total_support
        # )

        # weighted_recall = (
        #     sum(r * s for r, s in zip(recalls, numGtCum)).item()
        #     / total_support
        # )

        # weighted_fscore = (
        #     sum(f * s for f, s in zip(fscores, numGtCum)).item()
        #     / total_support
        # )
        ...
        #     f"f{beta}-score": weighted_fscore,
        #     "support": total_support,
        # }
