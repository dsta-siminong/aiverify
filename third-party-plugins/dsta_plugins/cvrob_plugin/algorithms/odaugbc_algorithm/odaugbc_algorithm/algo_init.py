...
from odaugbc_algorithm.algo import Plugin
...
class AlgoInit:
    ...
    def _generate_output_file(self, results, output_path: Path) -> None:
        ...
        try:
            ...
            artifact_list = self._populate_all_image_urls(results)
    ...
    def _populate_all_image_urls(self, data):
        ...
        # td = data["results"]["display_info"]z
        ...
        for result in data["results"]:
            ...
            for info in result.get("display_info", {}).values():
                for item in info:
                    if str(item).endswith((".png", ".jpg", ".jpeg", ".html")):
                        image_urls.append((item))
            for info in result.get("confusion_matrix", {}).values():
                for item in info:
                    if str(item).endswith((".png", ".jpg", ".jpeg", ".html")):
                        image_urls.append((item))
            for k,v in result['plot_paths'].items():
                if k == "class_plot":
                    for _, l in v.items():
                        image_urls += l
                elif k in ["matplotlib_image_path", "plotly_image_path"]:
                    image_urls.append(v)
                elif k == "map_plot":
                    image_urls += v
                else:
            ...
            for k,v in result['coco_graphs'].items():
                image_urls += v
            for k,v in result['coco_graphs_overall'].items():
                for _,v1 in v.items():
                    image_urls.append(v1)
        return image_urls