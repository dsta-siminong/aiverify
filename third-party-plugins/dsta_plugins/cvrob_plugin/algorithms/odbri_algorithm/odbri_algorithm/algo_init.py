...
from odbri_algorithm.algo import Plugin
...
class AlgoInit:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._input_arguments = kwargs#: Dict = input_arguments if input_arguments else {}
        self._model_type: ModelType = kwargs.get("model_type")

        if self._requires_ground_truth:

            self._ground_truth_path: str = str(ground_truth_path)
            self._ground_truth: str = kwargs.get("ground_truth")
        else:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            log_path = Path(self._base_path) / "algo_debug.log"
            file_handler = logging.FileHandler(str(log_path))#(filename="test.log")
            ...
            if self._run_as_pipeline:
                ...
                self._initial_data_instance = copy.deepcopy(
                    self._data_instance)
                self._initial_model_instance = copy.deepcopy(
                    self._model_instance)
            ...
            if self._data_instance and self._model_instance:
                ...
                raise RuntimeError(
                    "ERROR: Unable to get data or model instances")
    ...
    def _generate_output_file(self, results, output_path: Path) -> None:
        ...
        """
        debug_path = Path("output") / "algo_debug.txt"
        debug_path.parent.mkdir(parents=True, exist_ok=True)

        try:
            with open(debug_path, "w") as f:
                f.write("ENTERED _generate_output_file\n")
            ...
            # Add additional arguments used in input.schema or passed to the plugin over here
            algorithmArgs={
            ...
            artifact_list = []
            # Debug print: what keys are in results
            print(">>> _generate_output_file called, results keys:", list(results.keys()))
            with open(debug_path, "a") as f:
                f.write(f">>> _generate_output_file called, results keys: {list(results.keys())}")
            ...
            with open(debug_path, "a") as f:
                f.write(f">>> artifact_list before creating ITestResult: {artifact_list}")
            #self._logger_instance.debug(f">>> artifact_list before creating ITestResult: {artifact_list}")


            image_paths = self._populate_all_image_urls(results)
            print(">>> image paths:", image_paths)
            with open(debug_path, "a") as f:
                f.write(f">>> artifact_list before creating ITestResult: {image_paths}")

            artifact_list += image_paths
            # print(">>> artifact_list2 before creating ITestResult:", artifact_list)
            # with open(debug_path, "a") as f:
            #     f.write(f">>> artifact_list2 before creating ITestResult: {artifact_list}")
            #self._logger_instance.debug(f">>> artifact_list2 before creating ITestResult: {artifact_list}")
            ...
            # Debug print: what is actually in the artifacts field of the output object
            print(">>> output JSON artifacts field:", output.artifacts)
            with open(debug_path, "a") as f:
                f.write(f">>> output JSON artifacts field: {output.artifacts}")
            #self._logger_instance.debug(f">>> output JSON artifacts field: {output.artifacts}")
    ...
    def _populate_all_image_urls(self, data):
        ...
        for result in data["display_info"]:
